...
export function validateReasonForAdmission(text) {
...
export function patientGenderLabel(value) {
  const match = PATIENT_GENDER_OPTIONS.find((o) => o.value === value);
  return match?.label || String(value || '').trim() || '';
}

/** @returns {string} Empty string when valid, otherwise an error message. */
export function validatePatientBirthDate(iso) {
