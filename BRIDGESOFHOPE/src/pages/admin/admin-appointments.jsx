...
export default function AdminAppointmentsPage() {
  ...
  useEffect(() => {
    if (!dayAppointmentsModal) return;
    const onKey = (e) => {
  ...
  }, [selectedDays, startTime, endTime, scheduleLoaded]);

  const decide = (row, action) => {
    if (!row?.id) return;
    let nextStatus = 'Requested';
    let nextConfirmedDate = row.confirmedDate || '';
    let nextConfirmedTime = row.confirmedTime || '';
    if (action === 'Approved') {
      nextStatus = 'Approved';
      nextConfirmedDate = row.preferredDate || '';
      nextConfirmedTime = row.preferredTime || '';
      ...
      nextConfirmedDate = '';
      nextConfirmedTime = '';
      updateVisitationRequest(row.id, { status: nextStatus, confirmedDate: '', confirmedTime: '' });
    } else {
      const date = window.prompt('Rescheduled date (YYYY-MM-DD):', row.preferredDate || '');
      if (!date) return;
      const time = window.prompt('Rescheduled time (HH:mm):', row.preferredTime || '');
      if (!time) return;
      nextStatus = 'Rescheduled';
      nextConfirmedDate = date;
      nextConfirmedTime = time;
      updateVisitationRequest(row.id, { status: nextStatus, confirmedDate: nextConfirmedDate, confirmedTime: nextConfirmedTime });
    }
    if (isSupabaseConfigured()) {
      void supabase
        .from('visitation_requests')
    ...
    setQueue((prev) => prev.map((item) => (
      String(item.id) === String(row.id)
        ? { ...item, status: nextStatus, confirmedDate: nextConfirmedDate, confirmedTime: nextConfirmedTime }
        : item
    )));
    window.dispatchEvent(new Event('storage'));
    window.dispatchEvent(new Event(APP_DATA_REFRESH));
  ...
  const filteredQueue = selectedCalendarDate
    ? visibleQueue.filter((row) => String(row.preferredDate || '') === selectedCalendarDate || String(row.confirmedDate || '') === selectedCalendarDate)
    : visibleQueue;
  const requestCountByDate = useMemo(() => {
    const map = new Map();
    visibleQueue.forEach((row) => {
      const dates = new Set(
        [String(row.preferredDate || '').trim(), String(row.confirmedDate || '').trim()].filter(Boolean)
      );
      dates.forEach((d) => {
  ...
  const visitationsOnIso = (iso) =>
    visibleQueue.filter(
      (row) => String(row.preferredDate || '') === iso || String(row.confirmedDate || '') === iso
    );
  ...
  return (
    <div className="ap-outer" style={{ display: 'flex', minHeight: '100vh', background: '#F8F9FD', fontFamily: "'Inter', sans-serif", color: '#1B2559' }}>
      <style>{`
        ...
        .ap-day-modal-close:hover { background: #e2e8f0; color: #0f172a; }
              ...
              <div key={row.id} className="admin-queue-card">
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#64748B' }}>
                    Preferred: {row.preferredDate || 'N/A'} {row.preferredTime || ''}
                    {row.confirmedDate ? ` · Confirmed: ${row.confirmedDate} ${row.confirmedTime || ''}` : ''}
                  </div>
                </div>
