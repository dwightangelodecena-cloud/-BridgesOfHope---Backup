      ...
      try {
        if (!isSupabaseConfigured()) {
          ...
          const mapped = (patients || []).map((p) => {
            ...
            return {
              patientId: p.id,
              patientName: p.name || p.full_name || 'Resident',
              checkedTiles: tiles,
              baseProgress,
    ...
    <div className="rr-outer" style={{ display: 'flex', minHeight: '100vh', background: '#F8F9FD', fontFamily: "'Inter', sans-serif", color: '#1B2559' }}>
      ...
      <aside className="desktop-sidebar" onClick={() => setIsExpanded((prev) => !prev)}>
        ...
        <div className="sidebar-footer">
          <div className="sidebar-nav-item" onClick={(e) => { e.stopPropagation(); navigate('/admin-profile'); }}><div className="icon-box inactive"><User size={22} /></div><span className="sidebar-label">Profile & Security</span></div>
      ...
      <main className="rr-main">
        ...
        <div className="rr-card" style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 700 }}>Residents tracked: {rows.length}</div>
        ...
        <div className="rr-card">
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#323D4E', color: 'white' }}>
                  {['Resident', 'Checked Tiles', 'Ladder position', `Base (${PROGRESS_PER_TILE}%/tile)`, 'Manual override', 'Final progress', 'Behavior / intervention', 'Governance link', 'Last Checked'].map((h, i) => (
