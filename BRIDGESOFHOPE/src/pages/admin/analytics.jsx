...
function mapProgramKey(concern) {
    ...
    if (c.includes('physical') || c.includes('physio') || (c.includes('therapy') && !c.includes('mental'))) return 'physical';
    if (c.includes('mental') || c.includes('anxiety') || c.includes('depression') || c.includes('psych')) return 'mental';
    if (c.includes('substance') || c.includes('drug') || c.includes('alcohol') || c.includes('abuse') || c.includes('gambling')) return 'substance';
    return 'other';
}

function programMatchesFilter(rawKey, filterVal) {
    if (filterVal === 'all') return true;
    const pk = rawKey === 'other' ? 'mental' : rawKey;
    if (filterVal === 'mental') return rawKey === 'mental' || rawKey === 'other';
    return pk === filterVal;
}
...
const Analytics = () => {
    ...
    const successRate = metrics.total > 0 ? Math.round((metrics.approved / metrics.total) * 100) : 0;
    ...
    const barCounts = useMemo(() => {
        const c = { substance: 0, physical: 0, mental: 0 };
        filteredPatients.forEach((p) => {
            const pk = p.programKeyRaw === 'other' ? 'mental' : p.programKeyRaw;
    ...
    const insights = useMemo(() => {
        ...
        const progKeys = [
            { key: 'substance', label: 'Substance Abuse' },
            { key: 'physical', label: 'Physical Therapy' },
        ...
        progKeys.forEach(({ key, label }) => {
            const cohort = normalizedPatients.filter((p) => {
                ...
                if (key === 'mental') return raw === 'mental' || raw === 'other';
                return raw === key;
            });
    ...
    return (
        <div
            ...
            style={{
                display: 'flex',
                minHeight: '100vh',
                background: 'linear-gradient(165deg, #eef2ff 0%, #f8f9fd 38%, #fff7f4 100%)',
        ...
        >
            ...
            <div className="db-mobile-only db-mobile-top-bar">
                ...
                <div style={{ width: 36, height: 36, background: 'linear-gradient(145deg, #F54E25, #ff8a65)', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>JD</div>
            </div>
                ...
                <div className="analytics-print-root" style={{ maxWidth: 'min(1920px, 100%)', margin: '0 auto', width: '100%' }}>
                    ...
                    <div className="stats-row">
                        ...
                        <div className="stat-box stat-box--t4">
                            ...
                            <div className="stat-val-s">{successRate}%</div>
                        </div>
                        ...
                        <div className="chart-box">
                            ...
                            <div className="analytics-chart-wrap bar-chart-frame">
                                ...
                                <svg width="100%" height="220" viewBox="0 0 500 200" preserveAspectRatio="xMidYMid meet">
                                    ...
                                    <rect x="70" y={150 - barScale(barCounts.substance)} width="90" height={barScale(barCounts.substance)} fill="url(#barGrad)" rx="6" />
                                    <rect x="210" y={150 - barScale(barCounts.physical)} width="90" height={barScale(barCounts.physical)} fill="url(#barGrad)" rx="6" />
            ...
            <div className="db-mobile-only db-mobile-bottom-nav">
                ...
                <div className="mob-nav-item active">
                    <div style={{ background: 'linear-gradient(145deg, #F54E25, #ff8a65)', padding: 10, borderRadius: 10, display: 'flex' }}>
