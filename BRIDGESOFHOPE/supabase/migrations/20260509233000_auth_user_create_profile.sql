/*
  Family (and other) self-signup: client-side profiles upsert runs without a session when
  "Confirm email" is enabled, so RLS blocks insert. Create profile from auth.users metadata
  in a SECURITY DEFINER trigger (runs with definer privileges; not subject to RLS).
*/
