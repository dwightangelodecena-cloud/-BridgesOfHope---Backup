-- Family discharge requests: pending queue for assigned program staff only.
-- Admin/nurse retain access to processed rows; not the pending family queue.
...
as $$
  select lower(coalesce(
    ...
    ''
  )) in ('program', 'case_load_manager', 'case load manager', 'case manager');
$$;

comment on function public.is_program_account () is
  'Program workspace role from JWT metadata (no profiles read).';
